...
function setup() {
    ...
    general_files.forEach(img => {
        if (img.name.toLowerCase() != "taskmaster") {
            $("#div-basic-controls").append(" ").append('<button class="btn btn-primary special-button" onclick="showImage(event)" data-root="./data/' + img.file_source + '">Show ' + img.name + '</button>')
        }
    })
